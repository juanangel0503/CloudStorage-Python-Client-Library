...
def install_storage_package(executable_location, environment):
    if environment == 'test':
        check_call([executable_location, 'install', 'storage-nspkg'])
        check_call([executable_location, 'install', 'storage-common', '-i',
                    'https://testpypi.python.org/pypi', '--no-deps'])
        check_call([executable_location, 'install', 'storage-blob', '-i',
                    'https://testpypi.python.org/pypi', '--no-deps'])
        check_call([executable_location, 'install', 'storage-file', '-i',
                    'https://testpypi.python.org/pypi', '--no-deps'])
        check_call([executable_location, 'install', 'storage-queue', '-i',
                    'https://testpypi.python.org/pypi', '--no-deps'])
    elif environment == 'prod':
        check_call([executable_location, 'install', 'storage-blob', '--no-cache-dir'])
        check_call([executable_location, 'install', 'storage-file', '--no-cache-dir'])
        check_call([executable_location, 'install', 'storage-queue', '--no-cache-dir'])
    else:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Validate CloudStorage packages.')
